library(tidyverse)
bed_path="../resource/gencode.v19.comprehensive.ucsc.bed"
output="gencode.v19.comprehensive.ucsc.nonnmd.bed"

# alt chr has >7 cols but no probelm here
bed = read_tsv(bed_path,comment="#",col_types="cddc_c",col_names=c("chr","sta","end","enst","str")) %>%
...
# remove genes at PAR region automatically (those genes have 2 * n of exons here => no last, penultimate exon here
...
last_penu_1ex %>%
    ...
    write_tsv(output,quote=F,col_names=F)
